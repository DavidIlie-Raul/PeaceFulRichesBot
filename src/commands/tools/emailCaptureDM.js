...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("messageall-with-email-capture")
    .setDescription(
      "Send a DM to all members of the current server and attach an email capture widget to said message"
    )
  ...
  async execute(interaction, client) {
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        ...
        .setLabel("Sign up to our Newsletter here!")
        .setStyle("3")
    );
    const member =
      interaction.member ||
